...
MJC 26Aug2011 v1.0.0: version 11 update.
*/
...
program define survsim
	...
	/* Error checks */
		...
		if "`tde'"!="" & "`mixture'"!="" {
			di as error "time-dependent effects not allowed under mixture model"
			exit 198
		}
	...
	/* Equations for N-R */
		...
		/* Standard Weibull/Gompertz */
			if "`model'"=="weibull" | "`model'"=="gompertz" {
	...
	/* Standard Weibull/Gompertz calculations OR Starting values */
		...
		if "`model'"!="cr" {
			if "`dist'"=="weibull" {
				qui gen double `nr_time' 		= (-ln(`u')*(`gammastart' `tde_linpred')/(`lambdastart'*`gammastart' `cov_linpred'))^(1/(`gammastart' `tde_linpred')) if `use'
