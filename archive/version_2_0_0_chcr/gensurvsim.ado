...
program define gensurvsim
	...
	// Core stuff
		...
		if "`loghazard'"!="" | "`hazard'"!="" | ("`mixture'"!="" & "`tde'"!="") {
			...
			if "`mixture'"=="" {
				...
				foreach varname in `allvars' {
					mata: st_local("test",strofreal(regexm("`tempcumhaz'","`varname'"))) 
					if `test' {
						di "`varname'"
		...
		if ("`cumhazard'"!="" | "`logcumhazard'"!="") {
			...
			if "`tde'"!="" {
				local temptde temptde(`tdexb')
				if "`tdefunction'"=="" local tdefunction t
				else mata: st_local("tdefunction1",subinstr(st_local("tdefunction"),"#t","t"))
				local tempcumhaz (`tempcumhaz') :* exp(tdexb :* (`tdefunction1'))
				
				if "`tdefunction'"=="" local tdefunction0 enter
				else mata: st_local("tdefunction0",subinstr(st_local("tdefunction"),"#t","enter"))
