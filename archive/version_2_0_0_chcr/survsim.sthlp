...
{synopt:{opt centol(real)}}set the tolerance for the Newton-Raphson scheme, default is 1E-05{p_end}
...
{synopt:{opt nodes(#)}}number of Gauss-Legendre quadrature nodes, default 15{p_end}
...
{synopt:{opt mint:ime(#)}}minimum time for use in root finding, default 1E-05{p_end}
{synoptline}
...
{pstd}{cmd:survsim} simulates survival times from parametric distributions. Distributions include the exponential, Gompertz and Weibull. Newton-Raphson 
...
model covariates are interacted with time. Baseline covariates can be included in all models. {it:newvarname1} specifies the new variable name to contain the 
...
is specified. Finally, a user-defined [log] baseline hazard function can be specified, in Mata code, with survival times generated using a combination of Gaussian 
quadrature and root finding techniques.{p_end}
...
{phang}{opt maxtime(#)} specify an administrative censoring time. Two new varnames must be specified, the second to contain the event indicator.{p_end}
...
{phang}{opt centol(real)} specifies the tolerance of the Newton-Raphson scheme when using the {bf:mixture} or {bf:cr} models. Default is 1E-05.{p_end}
...
{dlgtab:User defined [log] baseline hazard function}
...
Time must be entered as #t. See examples below.{p_end}
...
{phang}{opt nodes(#)} defines the number of Gauss-Legendre quadrature points used to evaluate the cumulative hazard function in the simulation process. 
Default is 15. This should be increased to assess the stability of the simulation process.{p_end}
...
{phang}{opt mintime(#)} defines the minimum possible simulated survival time passed to {cmd:mm_root()}. Default is 1E-05. See {helpb moremata} for more details.{p_end}
...
{phang}{stata "survsim stime3, mixture lambdas(0.1 0.05) gammas(1 1.5) pmix(0.5)":. survsim stime3, mixture lambdas(0.1 0.05) gammas(1 1.5) pmix(0.5)}{p_end}
...
{phang}Crowther MJ and Lamber PC. {browse "http://leicester.academia.edu/MichaelCrowther/Papers/1783366/Simulating_complex_survival_data":Simulating complex survival data.}{it: The Stata Journal}, 2012, (In Press).{p_end}
