*! version 1.2.0 17oct2012 MJC

/*
History
MJC 17oct2012 version 1.2.0 - maxtime() added to specify maximum generated survival time and event indicator
							...
							- default centol() changed to 1E-05
							- varnames can now appear in loghazard()/hazard() allowing time-dependent covariates
							- mixture models now use Brent method -> much more reliable than NR, and allow tde's
...
//!! check cumh scale with tde's
//-> tolerances and mintime can effect bias!!!!!
...
program define survsim
	...
	syntax newvarname(min=1 max=2), 										///
									...
									/* Mixture */							///
										...
										BRENT								///	-Use Brent's univariate root finder
																			///
									/* Competing risks */					///
									...
									/* Newton-Raphson scheme */				///
										CENTOL(real 1E-08)					///	-Tolerance of Newton-Raphson iterations-
	...
	//call competing risks user-defined subroutine and exit	
	local store `0'
	local 0 `store'
	syntax newvarname(min=1 max=2) , [h1(string) logh1(string) cumh1(string) logcumh1(string) *]
	if ("`logh1'"!="" | "`h1'"!="" | "`logcumh1'"!="" | "`cumh1'"!="") {
		gensurvsimcr `store'
		exit
	}
