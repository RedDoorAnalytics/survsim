...
mata
function gensurvsim_func(t,U,nodes,weights,xb,enter $mmrootsyntax2)
{
...
function gensurvsim_func_tde(t,U,nodes,weights,xb,enter,tdexb $mmrootsyntax2)
{
	tnodes = (t:-enter):*0.5:*nodes :+ (t:+enter):/2
	tweights = (t:-enter):*weights:/2
	cumhaz_nodes = $cumhaz
...
function gensurvsim_func_ch(t,U,xb,enter $mmrootsyntax2)
{
...
void gensurvsim_mata($overallsyntax1)
{
	...
	if (st_local("ch")!="") {
		for (i=1;i<=N;i++){
			rc[i] = mm_root(t=1,&gensurvsim_func_ch(),mint,maxt,tol,maxit,U[i],xb[i],enter[i] $mmrootsyntax1) 	//lower,upper,tol,maxit
			time[i] = t
		}
	}
	else {
		if (st_local("temptde")=="") {
			for (i=1;i<=N;i++){
				rc[i] = mm_root(t=1,&gensurvsim_func(),mint,maxt,tol,maxit,U[i],nodes,weights,xb[i],enter[i] $mmrootsyntax1) 	//lower,upper,tol,maxit
				time[i] = t
			}
		}
		else {
			...
			for (i=1;i<=N;i++){
				rc[i] = mm_root(t=1,&gensurvsim_func_tde(),mint,maxt,tol,maxit,U[i],nodes,weights,xb[i],enter[i],tde[i]$mmrootsyntax1)
				time[i] = t
			}
		}
	}
