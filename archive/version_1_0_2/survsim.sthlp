...
{synopt:{opt l:ambdas(numlist)}} scale parameters for Weibull distributions.{p_end}
{synopt:{opt g:ammas(numlist)}} shape parameters for Weibull distributions.{p_end}
{synopt:{opt cov:ariates(vn # [vn # ...])}} baseline covariates and coefficient values to include in the survival model.{p_end}
{synopt:{opt tde(vn # [vn # ...])}} time dependent effects to interact with log time.{p_end}
...
is required when generating competing risks data to create the status indicator. Non-proportional hazards can be included under a standard 
Weibull model by interacting covariates with log time.{p_end}

{title:Options}

{phang}{opt n} specifies the number of survival times to generate. Default is _N.{p_end}

{phang}{opt lambdas} defines the scale parameters in the Weibull distributions. The number of values required depends on the model choice. 
Default is a single number corresponding to a standard Weibull. Under a {cmd:mixture} model 2 values are required. Under a competing risks
model, {cmd:cr}, the number of values are defined by {cmd:ncr}.{p_end}
...
{phang}{opt covariates} defines baseline covariates to be included in the linear predictor of the survival model, along with the value of the 
...
inputted for each competing risk, e.g. {cmd:covariates}(treat 0.5 0.5 0.5 0.5).{p_end}

{phang}{opt tde} defines covariates to be interacted with log time to create non-proportional hazards. This option is only available with a standard Weibull. 
Values should be entered as {cmd:tde}(trt 0.5), for example.{p_end}
 
{phang}{opt centol} specifies the tolerance of the Newton-Raphson scheme. Default is 0.0001.{p_end}

{dlgtab:Mixture model}

{phang}{opt mixture} specifies that survival times are simulated from a 2-component mixture Weibull distribution. {cmd:lambdas} and {cmd:gammas} 
must be of length 2.{p_end}
...
{pstd}On rare occasions the Newton-Raphson scheme may not always converge. The user should experiment with appropriate parameter values and tolerance 
levels.{p_end}
