...
{phang}{opt covariates} defines baseline covariates to be included in the linear predictor of the survival model, along with the value of the corresponding coefficient. For example
 a treatent variable coded 0/1 can be included, with a log hazard ratio of 0.5, by {cmd:covariates}(treat 0.5). Variable treat must be in the dataset before {cmd:survsim} is run. 
 If {cmd:cr} is used with {cmd:ncr(4)}, then a value of each covariate must be inputted for each competing risk, e.g. {cmd:covariates}(treat 0.5 0.5 0.5 0.5){p_end}
