...
program define gensurvsim
	...
	syntax newvarname(min=1 max=2), 										///
									...
									[										/// -Options-
										...
										MINTime(string)						///	-minimum time for Brents method-
										...
										ITerations(int 1000)				///
										CENTOL(real 1E-08)					///
										ENTER(varname)						///	-minimum limit for quadrature-
										*									///
									]
		...
		if "`enter'"!="" {
			...
			if `r(min)'<0 {
				di as error "enter(`enter') must be > 0"
				exit 198
			}
	...
	// Core stuff
		...
		if ("`mixture'"!="" & "`tde'"==""){
			...
			if "`dist'"=="exp" {
				local tempcumhaz -log(`pmix':*exp(-`l1':*t_cumhaz_temp) :+ (1:-`pmix'):*exp(-`l2':*t_cumhaz_temp))
			}
			else if "`dist'"=="weibull" {
				local tempcumhaz -log(`pmix':*exp(-`l1':*t_cumhaz_temp:^(`g1')) :+ (1:-`pmix'):*exp(-`l2':*t_cumhaz_temp:^(`g2')))
			}
			else {
				local tempcumhaz -log(`pmix':*exp((`l1':/`g1'):*(1:-exp(`g1':*t_cumhaz_temp))) :+  (1:-`pmix'):*exp((`l2':/`g2'):*(1:-exp(`g2':*t_cumhaz_temp))))
			}
		...
		if ("`cumhazard'"!="" | "`logcumhazard'"!="") {
			...
			mata: st_local("tempcumhaz",subinstr("`logcumhazard'`cumhazard'","#t","t_cumhaz_temp"))
			if "`logcumhazard'"!="" {
			...
			if "`tde'"!="" {
				...
				mata: st_local("tdefunction1",subinstr(st_local("tdefunction"),"#t","t_cumhaz_temp"))
			...
			if `nhazvars'==0 {
				//test
				mata: t_cumhaz_temp = xb = tdexb = 0.1
				cap mata: test1 = $cumhaz
				if _rc {
					di as error "Error in logcumhazard()/cumhazard()"
					exit 198
				}
				mata mata drop t_cumhaz_temp xb tdexb test1
			}		
	...
	//summarise _survsim_rc
		...
		qui count if `nvar1'==.
		if `r(N)'>0 di as error "Error: `r(N)' missing values generated in `nvar1'"
